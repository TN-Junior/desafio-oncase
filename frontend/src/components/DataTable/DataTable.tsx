      ...
      {selectedParticipant && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-4 rounded shadow-lg w-96">
            ...
            <input type="text" value={selectedParticipant.firstName} onChange={e => setSelectedParticipant({...selectedParticipant, firstName: e.target.value})} className="border p-2 w-full" />
            <input type="text" value={selectedParticipant.lastName} onChange={e => setSelectedParticipant({...selectedParticipant, lastName: e.target.value})} className="border p-2 w-full mt-2" />
            <input type="number" value={selectedParticipant.participation} onChange={e => setSelectedParticipant({...selectedParticipant, participation: Number(e.target.value)})} className="border p-2 w-full mt-2" />
            <div className="flex justify-between mt-4">
              <button onClick={handleEdit} className="bg-blue-500 text-white px-4 py-2 rounded">Salvar</button>
              <button onClick={() => handleDelete(Number(selectedParticipant.id))} className="bg-red-500 text-white px-4 py-2 rounded">Deletar</button>
              <button onClick={() => setSelectedParticipant(null)} className="bg-gray-500 text-white px-4 py-2 rounded">Cancelar</button>
            </div>
          </div>
        </div>
      )}
